...
function extractionPrompt(records: readonly SyncSourceRecord[]): string {
  ...
  return [
    ...
    '',
    '提取规则：',
    '- 先逐条分类为 explicit_task、inferred_task、supporting_action_candidate、thought_or_context、information 或 completed_action。',
    '- 只有 explicit_task、inferred_task 和 supporting_action_candidate 可以返回候选。',
    '- explicit_task 是来源直接表达的明确待办或尚未完成的行动承诺。',
    '- inferred_task 是能从来源目标合理推出的必要行动，summary 必须写明推断理由。',
    '- supporting_action_candidate 必须与来源中的当前目标直接相关，例如关联调研；不能只因主题相似就生成。',
    '- thought_or_context、information 和 completed_action 不返回候选；纯资讯、情绪记录、已经完成的行动和没有行动意图的观察都属于非候选。',
    '- #Ai使用 等主题标签不能单独作为行动证据。',
    '- 保留一次性或周期性意图、动作对象、预期成果和时间表达，不把周期性任务改写成一次性任务。',
    '- 同一来源中动作不同则分别返回，不要因为标题相似或为了减少数量强行合并。',
