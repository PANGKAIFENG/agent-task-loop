...
5. **需要 AI 时再用**：让思考教练启发你确认本周重点、整理任务字段，或授权 ATL Runner 做只读调研。
...
- 可以关联多个补充文件。上述四种格式可选择是否参与分析，图片、音视频等其他格式只在本地保存；
...
单个附件最大 50 MiB，单个文档最多提取 100,000 个字符；DOCX 解压后内容超过 64 MiB 会拒绝解析，PDF 最多解析 500 页和 100,000 个文本项。关联资料在你勾选并开始分析前不会解析。附件被移动或删除时，会议弹窗仍可打开并标出不可用文件，你可以移除后重新分析。
