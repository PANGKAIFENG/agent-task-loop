...
> 当前版本：`v0.2.0` MVP。适合同时推进多个项目、经常开启多个 AI 会话，又不希望遗漏后续动作的个人用户。
...
| ATL Obsidian 插件 | 确认任务、创建项目、管理后台执行和推荐看板样式 |
...
1. 打开 [Releases](https://github.com/PANGKAIFENG/agent-task-loop/releases)，下载 `agent-task-loop-v0.2.0.zip`。
...
- 已安装 TaskNotes，用于显示任务看板；
- 本机已有 Node.js `24+`；
- 本机已有 Claude Code，并已完成登录；
- Vault 中已有 ATL 的 `10_Tasks` 目录和任务总看板。

Node.js 和 Claude Code 是外部运行环境，不包含在 ATL 插件中。`v0.2.0` 会自动检测并连接它们，也会直接安装、更新或停用 ATL 自己的 macOS 后台任务；不再需要开发者克隆仓库、执行 `pnpm` 或手工配置调度器。
...
4. 用“选择文件夹”添加 Agent 允许读取的本地资料目录。
5. 状态显示“待安装”后，点击“启用后台执行”。
6. 状态变成“已就绪”后，可点击“立即试跑”检查一次队列。
7. 在“任务看板”中开启“ATL 紧凑卡片”，并按需点击“应用推荐布局”。
...
最简单的节奏是：每天集中清理一次 Inbox，随时查看 Ready 和 In Progress，再在 Review 中验收结果。安装完成后，这条日常流程不需要打开终端。
...
`v0.2.0` 聚焦“收集、确认、可视化管理、简单调研、人工验收”的个人闭环。暂不包含：
