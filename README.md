...
- **在 Obsidian 内确认任务**：配合 TaskNotes，从任务卡片文件菜单打开居中弹窗，选择或新建项目，补齐目标与验收标准后移入待执行，无需在日常确认时使用终端。

## 推荐用法：Obsidian + TaskNotes

Agent Task Loop 不替代 TaskNotes。两者的分工是：

| 组件 | 负责什么 |
| --- | --- |
| TaskNotes | 看板、任务卡片、筛选、日历和状态浏览 |
| ATL Obsidian 插件 | 居中确认弹窗、项目创建、任务确认入口和 Vault 授权 |
| ATL Core | 校验、状态迁移、文件移动、索引、审计和 Agent 执行 |

安装 ATL 插件后，在“设置 → Agent Task Loop”开启“允许 ATL 管理此 Vault”。之后从 TaskNotes 收件箱卡片的文件菜单选择“确认并移到待执行”，即可在 Obsidian 内完成项目、目标、验收标准、优先级和自动执行权限配置。

详细安装与操作说明见 [在 Obsidian 中确认 ATL 任务](docs/operations/obsidian-plugin.md)。
...
下面内容面向首次开发部署、自动调度和故障维护。完成 Obsidian 插件安装后，日常整理与确认任务不需要使用这些命令。
